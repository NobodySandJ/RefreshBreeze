document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('google-sheet-link').href = "https://script.google.com/macros/s/AKfycbxy8zZxX4s7AlvQ9NfBv4D_imH-z4BtIRUhYUbTLioJZMqjKPeSwpHkxRwVJQaPHaAF/exec";
    ...
    function handleDataResponse(data) {
        if (data && data.length > 0) {
            console.log("DATA MENTAH BARIS PERTAMA YANG DITERIMA:", data[0]);
        }
        loader.style.display = 'none';
        if (data.error) {
            loader.style.display = 'block';
            loader.innerHTML = `<p class="text-red-500">Error: ${data.error}</p>`;
            return;
        }
    ...
    function renderTable(data) {
        ...
        const parseDate = (dateString) => {
            ...
            if (parts.length === 3) {
                const timeParts = dateString.split(' ')[1] ? dateString.split(' ')[1].split(':') : [0, 0];
                return new Date(parts[2], parts[1] - 1, parts[0], timeParts[0] || 0, timeParts[1] || 0);
            }
            return new Date(dateString);
        };
        const sortedData = data.sort((a, b) => parseDate(b.Timestamp) - parseDate(a.Timestamp));

        sortedData.forEach((row) => {
            const tr = document.createElement('tr');
            const displayDate = parseDate(row.Timestamp);
            
            // PERUBAHAN DI SINI: row.items (dengan 's')
            tr.innerHTML = `
